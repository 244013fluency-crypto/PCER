...
import logging
logger = logging.getLogger("textattack.search_method")
...
class GeneticAlgorithm(PopulationBasedSearch, ABC):
    ...
    def perform_search(self, initial_result):
        #print(f"[DEBUG] initial_result goal_status: {initial_result.goal_status}")

        self._search_over = False
        population = self._initialize_population(initial_result, self.pop_size)
        #if not population:
            #print("[ERROR] Population is empty after initialization!")
        pop_size = len(population)
        current_score = initial_result.score
        #print(f"[DEBUG] Initialized population size: {pop_size}")
        for i in range(self.max_iters):
            #print(f"[DEBUG] Entering iteration {i}")
            population = sorted(population, key=lambda x: x.result.score, reverse=True)
            #logger.debug(f"[DEBUG] Iteration {i}/{self.max_iters}, best_score = {population[0].result.score:.4f}")
            #raise RuntimeError("[DEBUG] perform_search was called!")  # 立刻抛出异常确认是否调用
            if (
                self._search_over
