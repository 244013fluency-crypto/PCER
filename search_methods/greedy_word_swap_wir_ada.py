...
class GreedyWordSwapWIRAda(SearchMethod):
    ...
    def _update_index_scores(self, indices, scores, results):
        """根据每次变换的结果动态调整索引分数"""
        max_index = max(indices)  # 获取最大的索引值
        if max_index >= len(scores):
            # 如果最大的索引值超出了当前scores数组的大小，则扩展scores数组
        ...
        for idx, score, result in zip(indices, scores, results):
            ...
            # 计算更新后的索引得分：采用最近历史记录的平均得分改变
            if len(self.index_performance_history[idx]) > 5:  # 使用最近的五次记录
