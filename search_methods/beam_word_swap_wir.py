...
class BeamWordSwapWIR(SearchMethod):
    ...
    def __init__(self, wir_method="unk", unk_token="[UNK]", beam_width=8, min_beam_width=1, P_max=1.0):
        ...
        self.beam_width = beam_width  # 初始束宽
        self.max_beam_width = beam_width  # 最大束宽
        self.min_beam_width = min_beam_width  # 最小束宽
        self.P_max = P_max  # 初始的最大成功率
    ...
    # def update_P_max(self, score):
    #     # 更新最大成功率
    #     if score > self.P_max:
    #         self.P_max = score
    #
    # def calculate_P_s(self, score):
    #     # 计算成功率，归一化分数
    #     return (score - 0) / (self.P_max - 0)
    ...
    def _get_index_order(self, initial_text, max_len=-1):
        ...
        #
        #     if not delta_ps:
        #         delta_ps = [0.0] * len(softmax_saliency_scores)
        #
    ...
    def perform_search(self, initial_result):
        ...
        historical_best = initial_result  # 增加历史最优结果变量
        ...
        while not best_result.goal_status == GoalFunctionResultStatus.SUCCEEDED and iter_num < len(index_order):
            current_historical_best = historical_best  # 记录这轮迭代开始前的历史最优结果
            potential_next_beam = []
            for text in beam:
                ...
                #     print(f"No transformations found for index {index_order[iter_num]}. Skipping.")
                #     continue
                potential_next_beam += transformations
                # if not potential_next_beam:
                #     print("Warning: potential_next_beam is empty. Ending search.")
                #     return best_result
            ...
            if iter_num == 0:
                ...
                # successful_hypotheses = sum(
                #     scores[i] < prev_scores[beam[i]] for i in range(len(beam)))
                ...
                # 更新候选解的得分
                # prev_scores = {potential_next_beam[i]: scores[i] for i in range(len(potential_next_beam))}
